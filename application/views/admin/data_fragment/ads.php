 ...
 <table  id="" class="table table-striped table-bordered table-hover">
                                    ...
                                    <tbody>
                                       <?php 
                                       		...
                                       		$sql="SELECT `ID`, `Title`, `Description`, `Image`, `url`,`Status` FROM `Advertise` WHERE isActive=1 ORDER BY ID DESC ";
                                       		...
                                       		if($query){
                                       			while ($result = mysql_fetch_array($query->result_id)){
                                       				?>
			                                        	...
			                                        	<td ><a class="label label-danger" onclick="adsPublishFromTable('<?php echo $result['ID'];?>')">Unpublish</a></td>
			                                        	<?php }else{?>
			                                        	<td ><a class="label label-success" onclick="adsUnPublishFromTable('<?php echo $result['ID'];?>')"> Publish</a></td>
			                                        	<?php }?>
